...
class TestAbsLoss(TestCase):
	...
	# TODO The length of the array is less than 1.
...
class TestZeroOneLoss(TestCase):
    def test_simple(self):
		...
		# note: zero_one_loss NOT return sum
...
class TestInitializeZero(TestCase):
    def test_simple(self):
        ...
        t.call("initialize_zero") # check that the register a0 contains the correct array (hint: look at the check_array_pointer function in framework.py)
    ...
    def test_bad_length(self):
        ...
        t = AssemblyTest(self, "../coverage-src/initialize_zero.s")
        # set a0 to an invalid length of array
        t.input_scalar("a0", -1)
        # call the `initialize_zero` function
        t.call("initialize_zero")
        # generate the `assembly/TestInitializeZero_test_simple.s` file and run it through venus
        t.execute(code=36)

	# If malloc fails, this function terminates the program with exit code 27.
    def test_fail_malloc(self): 
        # load the test for initialize_zero.s
        t = AssemblyTest(self, "../coverage-src/initialize_zero.s")
        # set a0 to an invalid length of array
		# malloc fail, because the size bigger than heap
