...
map:
    ...
    # THINK: Why a0?
	# my answer: pass the value of crrent node to f
    ### YOUR CODE HERE ###
	lw a0, 0(s0)

    # Call the function in question on that value. DO NOT use a label (be prepared to answer why).
    # Hint: Where do we keep track of the function to call? Recall the parameters of "map".
	# my answer: s1, the address of function(square or decrement)
    ### YOUR CODE HERE ###
	jalr ra, s1, 0 # using rd to store return address(rd = ra = PC + 4 = next instrution = 'sw a0, 0(s0)')
	...
	# my answer: a1 keep stores the address of function(square or decrement)
    ### YOUR CODE HERE ###
	add a1, s1, x0 # avoid a1 be modiy? but even deleting this line, the process also runs with right answer, because we never change a1 before call other function

    # Recurse
    ### YOUR CODE HERE ###
	# in order to recurse, we should restore the value in stack by sp, otherwise we will lost the original data of ra,s0 and s1
	lw	ra, 0(sp)
	lw	s0, 4(sp)
	lw	s1, 8(sp) 
	addi sp, sp 12
