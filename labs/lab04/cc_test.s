# you should add helper_fn in '.global' to 
# test helper_fn in 'calling convention checker'
# because the calling convention checker will
# not test functions that are not declared .globl
...
next_test:
	# note: it is a loop NOT function, because jumping 'next_test'
	# by 'beq'(branch NOT jump instruction)
	# and 'next_test' haven't 'ret' instruction
...
pow:
    # BEGIN PROLOGUE
    # Need to save the calle saved register(s)
	addi sp sp -8
	sw ra 0(sp)
	sw s0 4(sp)
    # END PROLOGUE
...
pow_end:
    mv a0, s0
    # BEGIN EPILOGUE
    # Need to restore the calle saved register(s)
	lw ra 0(sp)
	lw s0 4(sp)
	addi sp sp 8
    # END EPILOGUE
...
inc_arr:
    # BEGIN PROLOGUE
    # What other registers need to be saved?
    addi sp, sp, -12
    sw ra, 0(sp)
    sw s0, 4(sp)
    sw s1, 8(sp)
...
inc_arr_loop:
	...
	# my answer: because 't1' is a temp value, but 't0' is counter
	# note: after calling helper_fn, we should assume 't0' has garbage,
	# because 'calling convention checker' will do this thing
	# that fill garbage in unused register
	#
    jal helper_fn
    # Restore t0
	lw t0 0(sp)
...
inc_arr_end:
    # BEGIN EPILOGUE
    # What other registers need to be restored?
    lw ra, 0(sp)
    lw s0, 4(sp)
    lw s1, 8(sp)
    addi sp, sp, 12
