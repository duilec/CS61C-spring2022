...
map:
	...
	jal x0, mapLoop		# jump to mapLoop
...
mapLoop:
    lw t1, 0(s0)      	# load the address of the array of current node into t1 -> it is loading address to t1
    lw t2, 4(s0)        # load the size of the node's array into t2 -> it is loading value to t2

	slli t3, t0, 2		# sizeof(int) == 4
    add t1, t1, t3      # offset the array address by the count
    lw a0, 0(t1)        # load the value at that address into a0

	addi sp, sp, -12	# because these will repeat using aftering calling 'f'(i.e. jalr s1) 
	sw t0, 0(sp)		
	sw t1, 4(sp)
	sw t2, 8(sp)
    ...
    lw s0, 8(sp)		# note: we will go to 'done' 5 times(because we have 5 node and go to 'map' 5 times)
    lw s1, 4(sp)
    lw ra, 0(sp)
    addi sp, sp, 12
	ret
